...
const ContentCardList = () => {
  ...
  const card = sortedContents.map((cont) => (
    <div
      key={cont._id}
      id={cont._id}
      className="bg-card min-h-[182px] h-fit z-0 hover:border hover:border-deep-secondary transition-all ease duration-500 relative mb-3 rounded-3xl border px-12 py-7 text-left shadow-lg hover:shadow-sm lg:px-7"
    >
      {/* Title and Author */}
      ...
      {/* Notes Section */}
      {cont.notes ? (
        <span className="text-sm font-light text-gray-400 mt-2"><ReadMore buttonClass={"text-muted-foreground font-semibold"} text={cont.notes} limit={10}/></span>
      ) : null}

      {/* Read Status Section" */}
      <div className="mt-3 flex items-center justify-between">
        <div className="rounded-lg font-semibold bg-accent-foreground py-1.5 px-3 text-xs text-secondary mr-3">
        ...
        <div className=" flex items-end justify-between">
          ...
          <div className="flex gap-2">
            ...
            {cont.sourceUrl && (
              ...
              >
                <svg
                  className="h-6 w-6 text-muted-foreground stroke-2"
                  viewBox="0 0 24 24"
                  fill="none"
            ...
            >
              <svg
                className="h-5 w-5 text-muted-foreground stroke-2"
                version="1.1"
                id="Capa_1"
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 25 25"
